...
void X86CpuDumpSummary(Timing *self, FILE *f)
{
	...
	/* Print statistics */
	printf("FastForwardInstructions = %lld\n", cpu->num_fast_forward_inst);
	printf("CommittedInstructions = %lld\n", cpu->num_committed_inst);
	printf("CommittedInstructionsPerCycle = %.4g\n", inst_per_cycle);
	printf("CommittedMicroInstructions = %lld\n", cpu->num_committed_uinst);
	printf("CommittedMicroInstructionsPerCycle = %.4g\n", uinst_per_cycle);
	printf("BranchPredictionAccuracy = %.4g\n", branch_acc);
