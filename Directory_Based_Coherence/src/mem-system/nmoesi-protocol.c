...
void mod_handler_nmoesi_load(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_LOAD_MISS)
	{
		...
		next_state = (stack->shared) ? cache_block_shared : cache_block_exclusive;
...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_PORT)
	{
		...
		if(!stack->directory_lock_waiting_start_cycle)
			stack->directory_lock_waiting_start_cycle = esim_cycle();
		...
		stack->directory_lock_waiting_end_cycle = esim_cycle();
		stack->directory_lock_waiting_cycle			= stack->directory_lock_waiting_end_cycle - stack->directory_lock_waiting_start_cycle;
		
		if(stack->directory_lock_waiting_cycle) mod_update_directory_lock_waiting_counters(mod, stack);
...
void mod_handler_nmoesi_read_request(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_UPDOWN_FINISH)
	{
		...
		else if (stack->reply_size > 8)
		{
			...
			// Using stack->mod just to confirm that the lower level module was the module that performed the data transfer from its end.
			stack->target_mod->data_transfer_updown_load_request++;
		}
		else 
		{
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_DOWNUP_WAIT_FOR_REQS)
	{
		...
		if (stack->peer)
		{
			...
			//------------------------------------------------------------
			// Updating statistics, first indicate that the requesting module received the data via peer transfer and second statistics that a down-up request sent data through peer transfer
			//------------------------------------------------------------
			stack->peer->peer_transfers++;
			stack->target_mod->peer_data_transfer_downup_load_request++;
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_DOWNUP_FINISH)
	{
		...
		if (stack->reply == reply_ack_data)
		{
			...
			if (stack->peer) 
			{
				 ...
				 * to owned */
				// To change to MOESI protocol the peer transfer results the new string in the owned state.
				// In addition, the data needs also be sent to lower level module in order to maintain the data consistency and coherency and also change the retain_owner flag to give up ownership.
				cache_set_block(target_mod->cache, stack->set, stack->way, 
				...
				// In case of MOESI protocol ownership has to be retained.
				ret->retain_owner = 1;
			}
			else
			{
				...
				// MOESI protocol this data will also be transferred to the lower level module to maintain coherency. In case of MOESI, this is case of owned state and hence this additional data transfer doesn't occur.
				// stack->mod indicates that the higher level module sent the data back to lower level module
				stack->target_mod->data_transfer_downup_load_request++;
		...
		else if (stack->reply == reply_none)
		{
			...
			if (stack->peer) 
			{
				...
				{
					/* Let the lower-level cache know not to delete the owner */
					// MOESI-Protocol, ownership need to be retained.
					ret->retain_owner = 1;
			...
			else 
			{
					...
					stack->state == cache_block_noncoherent)
				{
					...
					// stack->mod that this higher level module sent the data for a Load request.	
					stack->target_mod->data_transfer_downup_load_request++;
				}
				else 
				{
...
void mod_handler_nmoesi_write_request(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_WRITE_REQUEST_UPDOWN_FINISH)
	{
		...
		next_state = cache_block_exclusive;
	...
	if (event == EV_MOD_NMOESI_WRITE_REQUEST_DOWNUP)
	{
			...
			stack->state == cache_block_owned)
		{
			if (stack->peer) 
			{
				...
				//------------------------------------------------------------
				stack->peer->peer_transfers++;
				stack->target_mod->peer_data_transfer_downup_store_request++;
