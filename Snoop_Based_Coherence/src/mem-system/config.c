	...
	char *section)
{
	...
	/* Create MSHR. This is done for snoop based protocols only*/
	mod->mshr = xcalloc(mshr_size, sizeof(struct mshr_entry_t));
	...
	// Get the module Id
	mod_id = config_read_int(config, section, "ModId", 0);
...
static void mem_config_calculate_sub_block_sizes(void)
{
	...
	for (i = 0; i < list_count(mem_system->mod_list); i++)
	{
		...
		// TBD : DTS Check the number of upper level nodes
		mod->num_nodes = num_nodes;
		// TBD : DTS Modify Appropriately
